"""Bar plot of mean qa_score per model, husl palette.

x-axis : models present in results.jsonl
y-axis : mean qa_score (averaged across records for each model)
...
def make_bar(results_jsonl: Path, out_png: Path) -> Path:
    ...
    ax.set_ylim(0, 1.08)
    ax.set_ylabel("Mean qa_score (ratio accuracy)", fontsize=12)
