"""Offline unit tests for CodeEdit scoring helpers. No API keys, no network, no
CAD execution (those paths are exercised by the full prod run).

CodeEdit's headline metric is `norm_iou` — the headroom-normalized improvement of
the edited program over the unedited one toward the target — so these tests pin
its contract directly.
"""
...
def test_norm_iou_perfect_baseline():
    # baseline already perfect (no headroom): only a perfect model scores 1
    assert norm_iou(1.0, 1.0) == 1.0
    assert norm_iou(0.9, 1.0) == 0.0


def test_norm_iou_no_change_baseline_is_zero():
    # the unedited program (model_iou == baseline_iou) traverses no gap -> 0
    assert norm_iou(0.5, 0.5) == 0.0
    assert norm_iou(0.0, 0.0) == 0.0
